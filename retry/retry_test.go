...
func TestIsRetryableWithCustomFunc(t *testing.T) {
	// Custom function that retries all errors
	config := Config{
		MaxAttempts: 3,
	...
	if !config.IsRetryable(permanentErr) {
		t.Error("custom func should make all errors retryable")
	}

	// Custom function that retries specific errors
	...
	if !config2.IsRetryable(specificErr) {
		t.Error("specific error should be retryable")
	}
	if config2.IsRetryable(errors.New("other error")) {
