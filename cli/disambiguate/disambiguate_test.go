...
func TestFormatAmbiguousError(t *testing.T) {
	...
	if !containsString(err, "start") || !containsString(err, "status") {
		t.Errorf("error message should contain command names: %s", err)
	}
}

func TestFormatMatchNames(t *testing.T) {
	matches := []CommandMatch{
		{Command: &cobra.Command{Use: "start", Short: "Start a task"}},
		{Command: &cobra.Command{Use: "status", Short: "Show status"}},
	}

	result := FormatMatchNames(matches)
...
func TestFindPrefixMatchesInPath(t *testing.T) {
	root := newTestRoot()

	// Test finding subcommands via path search
	matches := FindPrefixMatchesInPath(root, "validate")

	if len(matches) != 1 {
		t.Errorf("expected 1 match for 'validate', got %d", len(matches))
	} else {
		if matches[0].Command.Name() != "validate" {
			t.Errorf("expected 'validate', got %q", matches[0].Command.Name())
		}
		// Check path includes parent
		if len(matches[0].Path) != 2 || matches[0].Path[0] != "config" {
			t.Errorf("expected path [config, validate], got %v", matches[0].Path)
		}
	}
}
